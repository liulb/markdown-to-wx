...
const calculateWordCount = () => {
  ...
  temp.innerHTML = convertedHtml.value

  // 移除不计入字数的元素
  const elementsToRemove = temp.querySelectorAll('style, script')
  ...
  wordCount.value = chineseChars.length + englishWords.length + numbers.length
}
...
// 监听 convertedHtml 变化后再计算字数（确保 DOM 已更新）
watch(convertedHtml, () => {
  setTimeout(() => {
    calculateWordCount()
  }, 0)
